  ...
  bot.onText(/\/start(.*)/, async (msg, match) => {
    try {
      ...
      if (payload.startsWith("getflac_")) {
        ...
        if (!premium) {
          await sendPremiumPaywall(bot, msg.chat.id, "Downloading FLAC files is a CC Premium feature.");
          return;
        }
      ...
      if (payload.startsWith("getog_")) {
        ...
        if (!premium) {
          await sendPremiumPaywall(bot, msg.chat.id, "Downloading OG files is a CC Premium feature.");
          return;
        }
  ...
  bot.on("callback_query", async (query) => {
    try {
      ...
      if (query.data === "do_subscribe") {
        ...
        const first = files[0]!;
        const label = first.normal.title || first.normal.fileName || `File 1`;
        await bot.sendMessage(chatId, `Send me the OG file for:\n\n"${label}" (1/${files.length})`);
        return;
      ...
      if (query.data === "admin_premiumlist") {
        ...
        for (const line of lines) {
          if ((current + line + "\n").length > 3900) { chunks.push(current); current = ""; }
          current += line + "\n";
        }
      ...
      if (query.data === "tag_mode_normal") {
        ...
        userStates.set(userId, { step: "tagmp3_collecting_fast", buttons: [], tagFiles: [] });
        try { await bot.deleteMessage(chatId, msgId); } catch { }
        await bot.sendMessage(chatId, "Send me all the audio files you want to tag, one after the other. Send /done when you're finished.");
        return;
      }
      ...
      if (query.data === "fp_give") {
        ...
        await db
          ...
          .onConflictDoUpdate({
            target: usersTable.userId,
            set: { isPremium: true, premiumFree: true, premiumExpiry: null },
          });
      ...
      if (query.data === "skip_buttons" || query.data === "done_buttons") {
        ...
        userStates.delete(userId);
        try { await bot.editMessageText("✅ All songs deleted.", { chat_id: chatId, message_id: msgId }); } catch { }
        ...
        userStates.delete(userId);
        try { await bot.editMessageText("✅ Done.", { chat_id: chatId, message_id: msgId }); } catch { }
        ...
        const existing = await db.select().from(inlineAudioFilesTable).where(eq(inlineAudioFilesTable.id, id)).limit(1);
    ...
    try {
      ...
      if (state?.step === "fileforward_awaiting_og") {
        ...
        if (nextIdx < files.length) {
          ...
          await bot.sendMessage(msg.chat.id, `Got it! Now send me the OG file for:\n\n"${label}" (${nextIdx + 1}/${files.length})`);
        } else {
      ...
      if (state?.step === "earlymusic_awaiting_flac") {
        ...
        if (nextIdx < files.length) {
          ...
          const next = files[nextIdx]!;
          const label = next.normal.title || next.normal.fileName || `File ${nextIdx + 1}`;
          await bot.sendMessage(msg.chat.id, `Got it! Now send me the FLAC file for:\n\n"${label}" (${nextIdx + 1}/${files.length})`);
        } else {
      ...
      if (state?.step === "removefile_searching") {
        ...
        const keyboard: TelegramBot.InlineKeyboardButton[][] = matches.map((r) => [
          { text: `❌ ${formatSongLabel(r)}`.slice(0, 60), callback_data: `rm_${r.id}` },
        ]);
      ...
      if (state?.step === "freepremium_give_awaiting_user") {
        ...
        if (!isNaN(numId)) {
          ...
          const found = await db.select().from(usersTable).where(eq(usersTable.username, input)).limit(1);
          if (found.length > 0) { targetId = found[0]!.userId; targetUsername = found[0]!.username; }
        }

        if (!targetId) {
          await bot.sendMessage(msg.chat.id, `Couldn't find user "${input}". They must have started the bot at least once, or send their numeric Telegram ID.`);
          return;
        }
        ...
        await bot.sendMessage(msg.chat.id, `Are you sure you want to give ${name} free CC Premium?`, {
          reply_markup: {
            inline_keyboard: [[
      ...
      if (state?.step === "freepremium_remove_awaiting_user") {
        ...
        if (!targetId) {
          await bot.sendMessage(msg.chat.id, `Couldn't find user "${input}".`);
          return;
        }

        await db.update(usersTable).set({ isPremium: false, premiumFree: false, premiumExpiry: null }).where(eq(usersTable.userId, targetId));
        const name = targetUsername ? `@${targetUsername}` : `User ${targetId}`;
        await bot.sendMessage(msg.chat.id, `✅ Premium removed from ${name}.`);
        try { await bot.sendMessage(targetId, "Your CC Premium has been removed by an admin."); } catch { }
        userStates.delete(userId);
        return;
      }

      if (state?.step === "freepremium_give_confirm") return;
      ...
      if (state?.step === "search_awaiting_query") {
        ...
        const rows =
          conditions.length > 0
            ? await db.select().from(inlineAudioFilesTable).where(and(...conditions)).orderBy(desc(inlineAudioFilesTable.addedAt)).limit(10)
            : [];

        try { await bot.deleteMessage(msg.chat.id, searchingMsg.message_id); } catch { }

        if (rows.length === 0) {
          await bot.sendMessage(msg.chat.id, `sorry gurl couldn't find any results for "${query}" 😔`);
        } else {
          await bot.sendMessage(msg.chat.id, `🎵 Results for "${query}":`);
          for (const row of rows) {
            try { await bot.sendAudio(msg.chat.id, row.fileId); } catch (e) { logger.warn({ e }, "Could not send audio"); }
          }
        }
        return;
      }
...
async function processTagNormal(bot: TelegramBot, token: string, chatId: number, userId: number, state: UserState) {
  ...
  try {
    ...
    if (processingMsg) { try { await bot.deleteMessage(chatId, processingMsg.message_id); } catch { } }
    ...
    await safeSendMessage(bot, chatId, `Done!${limitNote} Send another file to tag, or /done when finished.`);
  } catch (err) {
...
async function processTagFast(bot: TelegramBot, token: string, chatId: number, userId: number, state: UserState) {
  const files = state.tagFiles || [];
  const premium = await isUserPremium(userId);

  const processingMsg = await safeSendMessage(bot, chatId, `⏳ Tagging ${files.length} file${files.length === 1 ? "" : "s"}...`);
  ...
  for (const file of files) {
    if (!premium) {
      ...
      if (remaining <= 0) {
        await safeSendMessage(bot, chatId, `⚠️ You've hit your daily tag limit (${DAILY_TAG_LIMIT}/day). Upgrade to CC Premium for unlimited tagging!\n\n${successCount} file${successCount === 1 ? "" : "s"} tagged before limit.`);
        break;
      }
    }
...
async function broadcastEarlyMusic(bot: TelegramBot, token: string, adminChatId: number, adminUserId: number, state: UserState) {
  ...
  const recipients = await db.select().from(usersTable).where(eq(usersTable.earlyMusicEnabled, true));
  ...
  userStates.delete(adminUserId);
  await bot.sendMessage(adminChatId, `✅ Sent ${pairs.length} file${pairs.length === 1 ? "" : "s"} to ${premiumRecipients.length} subscriber${premiumRecipients.length === 1 ? "" : "s"}.`);
}
...
const BANNED_ERROR_PATTERNS = [
  ...
  "chat not found", "have no rights", "not enough rights", "user is deactivated",
  "the group chat was deleted", "need administrator rights", "channel private", "forbidden",
];
...
interface CopyResult { ok: boolean; banned: boolean; error?: string; }

async function copyMessageViaApi(token: string, chatId: string, fromChatId: number, messageId: number, replyMarkup?: object): Promise<CopyResult> {
  const body: Record<string, unknown> = { chat_id: chatId, from_chat_id: fromChatId, message_id: messageId };
...
async function sendToSelectedChats(bot: TelegramBot, token: string, state: UserState, adminChatId: number, selectedIds: Set<string>) {
  ...
  let successCount = 0, failCount = 0, removedCount = 0;
  ...
  for (const chatId of selectedIds) {
    ...
    if (result.ok) {
      ...
      try { await db.delete(connectedChatsTable).where(eq(connectedChatsTable.chatId, BigInt(chatId))); } catch (dbErr) { logger.error({ dbErr }, "Failed to remove banned chat"); }
    } else {
      failCount++;
...
async function sendFileForwardToChats(bot: TelegramBot, token: string, state: UserState, adminChatId: number, selectedIds: Set<string>) {
  ...
  for (const pair of pairs) {
    try {
      await db
        ...
        .onConflictDoUpdate({
          target: botFilesTable.fileUniqueId,
          set: { fileId: pair.normal.fileId, ogFileUniqueId: pair.og?.fileUniqueId || null, ogFileId: pair.og?.fileId || null },
        });
  ...
  const totalChats = selectedIds.size - removedChatIds.size;
  const parts: string[] = [`${pairs.length} file${pairs.length === 1 ? "" : "s"} sent to ${totalChats} chat${totalChats === 1 ? "" : "s"} (${successCount} total sends).`];
